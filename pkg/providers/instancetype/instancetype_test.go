...
func TestTranslate_CPUOnly(t *testing.T) {
	...
	if got := it.Capacity[corev1.ResourceEphemeralStorage]; got.Cmp(resource.MustParse("100Gi")) != 0 {
		t.Errorf("ephemeral-storage capacity = %q, want 100Gi (from disk)", got.String())
	}
...
func TestTranslate_GPUOnly(t *testing.T) {
	...
	if got := it.Capacity[corev1.ResourceEphemeralStorage]; got.Cmp(resource.MustParse("40Gi")) != 0 {
		t.Errorf("ephemeral-storage fallback = %q, want 40Gi", got.String())
	}
}
